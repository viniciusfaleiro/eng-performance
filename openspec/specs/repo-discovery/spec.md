...
## Purpose
Given an (organization, project) pair, discover that project's real Azure DevOps
repositories, diff them against the registry, and let an admin apply the diff — inserting
missing repositories (unmapped, with an automatically suggested production stage when
unambiguous) and removing registered repositories that no longer exist in Azure DevOps —
as a shortcut on top of the existing one-by-one manual registration, never a replacement
for it.
